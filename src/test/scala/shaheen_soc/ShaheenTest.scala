...
class ShaheenTest(c: ShaheenTop, filePath: String)
...
//  val insts = for (i <- fileData) yield java.lang.Long.parseLong(i, 16)
  val insts =
    for (i <- fileData) yield java.lang.Long.parseLong(i.substring(2), 16)
...
//  Driver(() => new ShaheenTop) {c => new ShaheenTest(c)}
  ...
  execute(
    Array("--generate-vcd-output", "on", "--target-dir", "build_dir"),
    () => new ShaheenTop()
  ) { c =>
    new ShaheenTest(c, "/home/usman/Documents/instructions.txt")
  }
}
